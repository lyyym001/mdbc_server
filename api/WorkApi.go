...
func (aa *WorkApi) Handle_onRequest10022(p *core.Player, data []byte) {
	...
	var response_data pb.Tcp_Tj3Data
	var response_dataInfo pb.Tcp_Tj3Info
...
func (aa *WorkApi) Handle_onRequest10021(p *core.Player, data []byte) {
	...
	json.Unmarshal(data, request_data)
	var response_data pb.Tcp_Tj2Data
	var response_dataInfo pb.Tcp_Tj2Info
	...
	response_data.MaxNumber = _maxCount
	response_data.MaxPage = _maxPage
	data1, _ := json.Marshal(response_data)
	fmt.Println(response_data)
...
func (aa *WorkApi) Handle_onRequest10020(p *core.Player, data []byte) {
	...
	json.Unmarshal(data, request_data)
	var response_data pb.Tcp_Tj1Data
	var response_dataInfo pb.Tcp_Tj1Info
	//条件
...
func (aa *WorkApi) Handle_onRequest10018(p *core.Player, data []byte) {
	...
	json.Unmarshal(data, request_data)
	var response_data pb.Tcp_ResponseRecord
	...
	for rows.Next() {
		if err := rows.Scan(&maxUniqueId); err == nil {
			response_data.Code = 1
			log.Println("[统计]老师开启一场交互课件(最大id)-Succ,作品ID=", request_data.Workid, ",作品名称=", request_data.Workname, ",时间=", request_data.Date)
		} else {
			log.Println("[统计]老师开启一场交互课件(最大id),作品ID=", request_data.Workid, ",作品名称=", request_data.Workname, ",时间=", request_data.Date, ",Err=", err)
			response_data.Code = 0
		}
	}

	if maxUniqueId.Valid != false {
		UniqueId = maxUniqueId.Int32 + 1
	}

	if response_data.Code == 1 {
		//插入
		stmt, err := db.Prepare("INSERT INTO tb_work (workid, workname,`date`,mode,partnumber,uniqueid,maxscore,score) VALUES (?,?,?,?,?,?,?,0);")
		defer stmt.Close()
		if err == nil {
			stmt.Exec(request_data.Workid, request_data.Workname, request_data.Date, request_data.Mode, request_data.Partnumber, UniqueId, request_data.MaxScore)
			response_data.Code = 1
			log.Println("[统计]老师开启一场交互课件(写入)-Succ")
		} else {
			log.Println("[统计]老师开启一场交互课件(写入)-err=", err)
			response_data.Code = 0
		}
	}
...
func (aa *WorkApi) Handle_onRequest10017(p *core.Player, data []byte) {
	...
	core.WorldMgrObj.MScene.Mode = request_data.Mode
	...
	//响应
	response_data.UNumber = len(core.WorldMgrObj.MScene.Players)      //参与人数
...
func (aa *WorkApi) Handle_onRequest10016(p *core.Player, data []byte) {
	...
	json.Unmarshal(data, request_data)
	var response_data pb.Tcp_QuestionInfo

	fmt.Println("[提交答案]", request_data)
	if core.WorldMgrObj.MScene != nil && core.WorldMgrObj.MScene.Questions != nil {

		if _, ok := core.WorldMgrObj.MScene.Questions[request_data.StepId]; ok {
			//已经被别人答了
			response_data.Code = 0
			data1, _ := json.Marshal(response_data)
			p.SendMsg(2, 10016, data1)
		} else {
			response_data.Code = 1
			...
			data1, _ := json.Marshal(response_data)
			core.WorldMgrObj.Toa(2, 10016, data1)
		}
	}
}
...
func (aa *WorkApi) Handle_onRequest10014(p *core.Player, data []byte) {
	...
	core.WorldMgrObj.Too(p.PID, 2, 10014, data)
...
func (aa *WorkApi) Handle_onRequest10012(p *core.Player, data []byte) {
	...
	core.WorldMgrObj.Toa(2, 10012, data)
...
func (aa *WorkApi) Handle_onRequest10013(p *core.Player, data []byte) {
	...
	core.WorldMgrObj.Too(p.PID, 2, 10013, data)
...
func (aa *WorkApi) Handle_onRequest10011(p *core.Player, data []byte) {
	...
	json.Unmarshal(data, request_data)
	core.WorldMgrObj.WorkLeave(p.PID, request_data.State)
...
func (aa *WorkApi) Handle_onRequest10007(p *core.Player, data []byte) {
	...
	//给参与作品的其他人转发消息
	core.WorldMgrObj.Too(p.PID, 2, 10007, data)
}
...
func (aa *WorkApi) Handle_onRequest10005(p *core.Player, data []byte) {
	...
	//给参与作品的其他人转发消息
	core.WorldMgrObj.Too(p.PID, 2, 10005, data)
}
...
func (aa *WorkApi) Handle_onRequest10004(p *core.Player, data []byte) {
	...
	//给参与作品的其他人转发消息
	core.WorldMgrObj.Too(p.PID, 2, 10004, data)
}
...
func (aa *WorkApi) Handle_onRequest10003(p *core.Player, data []byte) {
	...
	json.Unmarshal(data, request_data)
	if _, ok := core.WorldMgrObj.MScene.TakeObjects[request_data.ObjId]; ok {
		//资源脱离
		delete(core.WorldMgrObj.MScene.TakeObjects, request_data.ObjId)
	...
	//给参与作品的其他人转发消息
	core.WorldMgrObj.Too(p.PID, 2, 10003, data)
}
...
func (aa *WorkApi) Handle_onRequest10002(p *core.Player, data []byte) {
	...
	if _, ok := core.WorldMgrObj.MScene.TakeObjects[request_data.ObjId]; ok {
		//资源已经被拿取了
		if p != nil {
			p.SendMsg(2, 10006, data)
			fmt.Println("[资源拿起失败],id=", request_data.ObjId, "记录的拿起数据=", core.WorldMgrObj.MScene.TakeObjects)
		}
	} else {
		core.WorldMgrObj.MScene.TakeObjects[request_data.ObjId] = p.PID
		//给参与作品的其他人转发消息
		core.WorldMgrObj.Too(p.PID, 2, 10002, data)
...
func (aa *WorkApi) Handle_onRequest10001(p *core.Player, data []byte) {
	...
	//给参与作品的其他人转发消息
	core.WorldMgrObj.Too(p.PID, 2, 10001, data)
}
